...
def _wa_icon_link_html(phone_number: str, *, for_grid: bool = False) -> str:
    """WhatsApp icon control: opens ``https://wa.me/<digits>`` in WhatsApp."""
    url = whatsapp_me_url(phone_number or "")
    if not url:
        return ""
    ...
    return (
        f'<a href="{url_e}" class="{link_cls}" target="_blank" rel="noopener noreferrer" '
        'title="Open in WhatsApp" aria-label="Open in WhatsApp" onclick="event.stopPropagation()">'
        '<span class="sr-only">Open in WhatsApp</span>'
