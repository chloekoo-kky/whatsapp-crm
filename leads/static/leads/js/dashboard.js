(function () {
      if (window.__leadsDashboardBooted) return;
      window.__leadsDashboardBooted = true;
      const cfgEl = document.getElementById('dashboard-js-config');
      const dashboardJsConfig = JSON.parse((cfgEl && cfgEl.textContent) || '{}');
      ...
      let activeSearchRecordId = dashboardJsConfig.activeSearchRecordId != null ? dashboardJsConfig.activeSearchRecordId : null;
      /* Hardcoded URLs: avoids view-context + Gunicorn stale imports; matches leads/urls and Ninja mount. */
      const clinicApiDetailPrefix = "/api/clinics/";
      const clinicUpdateUrlTemplate = "/leads/api/clinic/__ID__/";
      const leadDeleteUrlTemplate = "/leads/api/clinic/__ID__/delete/";
      const leadVipUrlTemplate = "/leads/api/clinic/__ID__/very-important/";
      const leadConversationLogUrlTemplate = "/leads/api/clinic/__ID__/conversation-log/";
      const REQUIRE_WEBSITE_KEY = "clinic_crm_require_website";
      const LEADS_PER_PAGE_KEY = "clinic_crm_leads_per_page";
      const CHAT_INDICATOR_POLL_KEY = "clinic_crm_chat_indicator_poll";
      const CHAT_INDICATOR_POLL_MS = 15000;
      const VIEW_MODE_KEY = "clinic_crm_clinic_view";
      const LEAD_SORT_KEY = "clinic_crm_lead_sort";
      const shopTypesApiPath = "/leads/api/shop-types/";
      const shopTypeDeleteUrlTemplate = "/leads/api/shop-types/__ID__/";
      ...
      var MANUAL_PHONE_MAX = 8;
      var manualPhoneRowTpl = document.getElementById('manual-phone-row-template');
      ...
      const huntLimitInput = document.getElementById('hunt-limit-value');
      ...
      const requireWebsiteToggle = document.getElementById('hunt-require-website');
      bindHuntOptionToggle(requireWebsiteToggle, REQUIRE_WEBSITE_KEY);

      const chatIndicatorPollToggle = document.getElementById('chat-indicator-poll-toggle');
      ...
      let clinicSaveStatusTimer = null;
        ...
        document.querySelectorAll('.clinic-row[data-clinic-id="' + id + '"]').forEach(function (row) {
          ...
          if (d.whatsapp_dispatched != null) {
            ...
            if (row.classList.contains('clinic-card')) {
              ...
              row.classList.toggle('border-2', !!d.whatsapp_dispatched);
              row.classList.toggle('border-emerald-400', !!d.whatsapp_dispatched);
              row.classList.toggle('hover:border-emerald-500', !!d.whatsapp_dispatched);
              row.classList.toggle('border', !d.whatsapp_dispatched);
              row.classList.toggle('border-slate-200/90', !d.whatsapp_dispatched);
              row.classList.toggle('hover:border-slate-300', !d.whatsapp_dispatched);
            }
          }
      ...
      const selectAll = document.getElementById('select-all-clinics');
      ...
      var folderTotalPipelineCount = 0;
      ...
      var leadChatIndicatorPollTimer = null;
      var leadChatIndicatorSnapshot = '';
      ...
      var chooseBatchDialog = document.getElementById('choose-batch-dialog');
      ...
      const exportXlsxStatus = document.getElementById('export-xlsx-status');
      const exportXlsxIcon = document.getElementById('export-xlsx-icon');
      const exportXlsxSpinner = document.getElementById('export-xlsx-spinner');
      ...
      const backupAllIcon = document.getElementById('backup-all-icon');
      const backupAllSpinner = document.getElementById('backup-all-spinner');
      ...
      const restoreBackupIcon = document.getElementById('restore-backup-icon');
      const restoreBackupSpinner = document.getElementById('restore-backup-spinner');
      ...
      const clinicsPanelEl = document.getElementById('clinics-panel');
      if (clinicsPanelEl) {
      ...
      let leadSortMode = readLeadSortMode();
      ...
      function toggleLeadFilterButton(btn, activeClass) {
        if (!btn) return;
        const on = btn.getAttribute('aria-pressed') !== 'true';
        btn.setAttribute('aria-pressed', on ? 'true' : 'false');
        if (activeClass === 'amber') {
          btn.classList.toggle('border-amber-500', on);
          btn.classList.toggle('bg-amber-50', on);
          btn.classList.toggle('text-amber-950', on);
          btn.classList.toggle('shadow-inner', on);
          btn.classList.toggle('ring-1', on);
          btn.classList.toggle('ring-amber-400/50', on);
        } else if (activeClass === 'emerald') {
          btn.classList.toggle('border-emerald-500', on);
          btn.classList.toggle('bg-emerald-50', on);
          btn.classList.toggle('text-emerald-950', on);
          btn.classList.toggle('shadow-inner', on);
          btn.classList.toggle('ring-1', on);
          btn.classList.toggle('ring-emerald-400/50', on);
        }
      ...
      function refreshTableSearchClearVisibility() {
        ...
        if (!btn) return;
        var has = si && si.value.trim().length > 0;
        btn.hidden = !has;
      ...
      const LEAD_FILTER_TAGS_KEY = 'clinic_crm_lead_filter_tags';
      const LEAD_FILTER_TAG_MAX = 24;
      const LEAD_FILTER_TAG_LEN_MAX = 80;
        ...
        list.innerHTML = tags.map(function (tag) {
          ...
          return (
            '<span class="lead-filter-tag' + (active ? ' lead-filter-tag--active' : '') + '" data-tag="' + escapeLeadFilterTagHtml(tag) + '" role="listitem">' +
              '<button type="button" class="lead-filter-tag-apply" aria-pressed="' + (active ? 'true' : 'false') + '" title="Apply filter: ' + escapeLeadFilterTagHtml(tag) + '">' +
                escapeLeadFilterTagHtml(tag) +
              '</button>' +
              '<button type="button" class="lead-filter-tag-remove" aria-label="Remove tag ' + escapeLeadFilterTagHtml(tag) + '" title="Remove tag">×</button>' +
            '</span>'
          );
        }).join('');
        refreshLeadFilterTagSaveButton();
      ...
      const HUNT_KEYWORD_TAGS_KEY = 'clinic_crm_hunt_keyword_tags';
      const HUNT_KEYWORD_TAG_MAX = 24;
      const HUNT_KEYWORD_TAG_LEN_MAX = 120;
      ...
      const HUNT_EXCLUDE_TAGS_KEY = 'clinic_crm_hunt_exclude_keywords';
      const HUNT_EXCLUDE_TAG_MAX = 12;
      const HUNT_EXCLUDE_TAG_LEN_MAX = 80;
      ...
      var GLOBAL_SEARCH_MIN_LEN = 2;
      var globalSearchActive = false;
      var globalSearchQuery = '';
      var tabBeforeGlobalSearch = 'uncategorized';
      var globalSearchDebounceTimer = null;
      var globalSearchRequestId = 0;
      var pendingHighlightLeadId = null;
      ...
      function getLeadGroupTabLabel(tabId) {
        ...
        if (!btn) return want === 'uncategorized' ? 'Uncategorized' : 'folder';
        var labelWrap = btn.querySelector('.relative');
        if (labelWrap) {
          var clone = labelWrap.cloneNode(true);
          clone.querySelectorAll('.lead-group-count').forEach(function (el) { el.remove(); });
          var text = (clone.textContent || '').trim();
          if (text) return text;
        }
        return (btn.textContent || '').trim() || want;
      }
      ...
      var currentLeadGroupTabId = normalizeLeadGroupTabId(
      ...
      async function switchLeadGroupTab(groupId, opts) {
        ...
        try {
          ...
          console.error(err);
          window.alert('Could not load leads for this group.');
        } finally {
          leadGroupTabBusy = false;
        }
      }
      ...
      var pendingMoveLeadIds = [];
      var leadGroupMoveMenuOpen = false;
      var leadGroupMoveMenuAnchor = null;
      ...
      const editDialog = document.getElementById('clinic-edit-dialog');
      const editForm = document.getElementById('clinic-edit-form');
      const editErr = document.getElementById('clinic-edit-error');
      const editSaveBtn = document.getElementById('clinic-edit-save');
      const editSaveLabel = document.getElementById('clinic-edit-save-label');
      const editSaveSpin = document.getElementById('clinic-edit-save-spinner');
      ...
      const leadCreateDialog = document.getElementById('lead-create-dialog');
      const leadCreateForm = document.getElementById('lead-create-form');
      const leadCreateErr = document.getElementById('lead-create-error');
      const leadCreateSubmit = document.getElementById('lead-create-submit');
      const leadCreateSubmitLabel = document.getElementById('lead-create-submit-label');
      const leadCreateSubmitSpinner = document.getElementById('lead-create-submit-spinner');
      ...
      const leadConversationLogDialog = document.getElementById('lead-conversation-log-dialog');
      const leadConversationLogForm = document.getElementById('lead-conversation-log-form');
      const leadConversationLogErr = document.getElementById('lead-conversation-log-error');
      const leadConversationLogOk = document.getElementById('lead-conversation-log-ok');
      const leadConversationLogSubmitBtn = document.getElementById('lead-conversation-log-submit');
      const leadConversationLogSubmitLabel = document.getElementById('lead-conversation-log-submit-label');
      const leadConversationLogSubmitSpinner = document.getElementById('lead-conversation-log-submit-spinner');
      const leadConversationHistoryList = document.getElementById('lead-conversation-history-list');
      const leadConversationHistoryEmpty = document.getElementById('lead-conversation-history-empty');
      ...
      const bulkManualDialog = document.getElementById('bulk-manual-dialog');
      const bulkManualForm = document.getElementById('bulk-manual-form');
      const bulkManualErr = document.getElementById('bulk-manual-error');
      const bulkManualCount = document.getElementById('bulk-manual-count');
      const bulkManualSubmit = document.getElementById('bulk-manual-submit');
      const bulkManualSubmitLabel = document.getElementById('bulk-manual-submit-label');
      const bulkManualSpinner = document.getElementById('bulk-manual-spinner');
      ...
      const huntBtnLabel = document.getElementById('hunt-btn-label');
      ...
      const citySuggestBtn = document.getElementById('hunt-city-suggest');
      const cityInput = document.getElementById('city');
      const stateSelect = document.getElementById('hunt-state');
      let citySuggestIndex = 0;
      let citySuggestState = stateSelect ? stateSelect.value : '';
